...
df_nevent <- df %>% group_by(website, event, name) %>% 
  summarize('number' = max(event_n),
            'mean_dur' = mean(event_d))
...
```{r}
mdl_nsaccade <- lmer(number ~ website + (1|name), data = df_nevent[df_nevent$event == 'saccade',])

summary(mdl_nsaccade)
```

```{r}
ggplot(data = df_nevent[df_nevent$event == 'saccade',], aes(x = number)) +
  geom_histogram() + facet_wrap(~website) + theme_bw()
```
